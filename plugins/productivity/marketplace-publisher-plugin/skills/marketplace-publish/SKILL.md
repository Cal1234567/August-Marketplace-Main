...
4. Ensure the marketplace repository is available:
   - If `claude-plugins` is not present, clone `https://github.com/Cal1234567/claude-plugins`.
...
6. From the marketplace repository, run:
   ...
   ```bash
   node scripts/promote-skill.mjs --src "<skill-folder-or-SKILL.md>" --category "<category>" --tags "<comma-separated-tags>"
   ```
...
9. Report that marketplace updates land in about 30 seconds, teammates can install the skill via
   Browse plugins, and the repository is `https://github.com/Cal1234567/claude-plugins`.
