...
> **Build decision (current — matches the golden HARVEST summary):** this skill produces a **clean titled document, NOT a deck cover.** Page 1 is: page-header logo → **title** (`TitleSummary`, big bold) → first H1 (`EXECUTIVE SUMMARY`). **No hero banner, no "INVESTMENT SUMMARY" label, no facts table.** The script builds this by default.
>
> ```
> ┌─ AUGUST GROUP ─────────────┐   (logo PNG in page header)
> │  HARVEST CLEAN EATS        │   ← TitleSummary (big bold), top of page 1
> │ EXECUTIVE SUMMARY          │   ← SubheaderSummary (H1) + solid rule
> │ HARVEST Clean Eats ("HAR.. │
> │ COMPANY OVERVIEW           │
> │ ...                        │
> └────────────────────────────┘
> ```
>
> **The old Addepar/Form/Radiant deck cover (hero banner → gold "INVESTMENT SUMMARY" label → 4-column facts table) is preserved but OPT-IN** — set `"cover": true` in the spec to bring it back. The default and the golden reference do not use it. See §11.4.

If `"cover": true` is set, the deck cover stack is, top to bottom:
...
> **Core principle: section headers are not fixed and are not copied from Sisyphus.** They are generated dynamically to fit the **company type / sector / deal**. What stays constant is the *styling* (every section heading is `SubheaderSummary`/H1, every sub-heading is `Subheading2`/H2), the *voice & conventions* (§1), and the *end-matter* (**Key Risks → Considerations → Appendix → Disclaimer** — the golden order; **no separate Risk Factors section**). Everything between the title and the end-matter — which sections exist, what they're called, their order, and their sub-headings — is selected to suit the business.
>
...
- Always anchor with **Executive Summary** first and **Key Risks** near the end, then the **end-matter** last (Considerations → Appendix → Disclaimer).
...
9. **Key Risks** — each risk as a **bullet with a bold lead-in + colon** (golden form). *(If a deal warrants explicit mitigants, name it "Key Risks & Mitigants" and use an `h2` per risk + a `Key Mitigants:` line — but the golden uses the simpler bullet form.)*
10. **Considerations** — deal caveats (non-advised nature, what the equity is, valuation/multiple, disclosure asks, illiquidity), or the SPV/fee mechanics when known. *(In the fund variant this becomes a substantive risk-discussion section — see §4.2.)*
11. **Appendix** — numbered **Exhibits** (team & board bios, financing history, market charts, technical diagrams, peer tables).
12. **Disclaimer** — boilerplate (verbatim, §7; default = the AGC standard text). **No separate Risk Factors section by default.**
...
### 7.2 Risk Factors (operating-company variant) — verbatim *(OPT-IN — the golden omits this section)*
> The golden HARVEST summary has **no Risk Factors section**; it ends Considerations → Appendix → Disclaimer. Only include this block (`risk_factors`) if a specific deal asks for it.
...
### 7.3 Disclaimer — AGC standard (DEFAULT — used by the golden) — verbatim
Bullets carry **no trailing period** (matching the golden exactly). This is what the `disclaimer` block emits when no `variant` is given.
> - The information herein was prepared by The August Group Capital Limited ("AGC") and is derived from sources AGC believes to be reliable, including information received from the fund manager or company management. AGC makes no representations or warranties as to accuracy, completeness or reliability of such information, and will not be held liable for any inaccuracies or misprints derived from such information
> - This report is not intended to be, and should not be taken to be, an offer to sell or a solicitation of an offer to buy the investment or investments that are the subject of the report, or any other investment. Investing your money in investment products involves the risk of loss, and past performance of investment products is not a guarantee of future returns. Investments in private funds are subject to a number of risks, including liquidity risk and the risk of loss of capital, and are designed for investors who understand and are able to withstand the risks
> - This report is confidential and intended solely for the use of the individual or entity to whom it is addressed. Any unauthorized review, use, disclosure, or distribution is prohibited. If you are not the intended recipient, please contact the sender and destroy any copies that may have been inadvertently sent to you
> - AGC is registered as a portfolio manager in Alberta, British Columbia, Manitoba, Ontario, and Quebec, and is registered as an investment adviser in the United States. The investment or investments that are the subject of this report are not affiliated with AGC

### 7.4 Disclaimer — short (legacy operating-company / Form & Radiant) — verbatim
...
### 7.5 Disclaimer — long (legacy Addepar / fund variant) — verbatim
...
- [ ] Clean first page (§3 / §11.4): logo header → `TitleSummary` title (big bold) → first H1 (`EXECUTIVE SUMMARY`). **No** hero banner, "INVESTMENT SUMMARY" label, or facts table unless `"cover": true` was explicitly requested.
...
- [ ] **Key Risks** (operating co., golden = bullets with bold lead-ins) or substantive **Considerations** (fund) present and candid — not softened.
...
- [ ] End-matter in golden order: **Key Risks → Considerations → Appendix → Disclaimer** (default = AGC standard text, §7.3). **No Risk Factors section** unless a deal explicitly asks for it.
...
TITLE     Calibre Semibold black, top of page 1 (no banner/label/facts by default; "cover":true re-adds them)
...
ENDMATTER Key Risks → Considerations → Appendix → Disclaimer (AGC standard, verbatim; no Risk Factors by default)
```
```
SECTIONS — DYNAMIC, chosen per company type. NOT a fixed list. See §4.
Fixed anchors only:  Executive Summary (first) ... Key Risks (late)
                     ... Considerations -> Appendix -> Disclaimer (end, verbatim; no Risk Factors by default)
...
· Key Risks · Considerations · Appendix (Exhibits) · Disclaimer
...
- End-matter: **Key Risks → Considerations → Appendix → Disclaimer** (verbatim, §7) — use the **operating-company** Considerations (deal caveats or SPV/fee mechanics) and the **AGC standard disclaimer** (§7.3) by default. **No Risk Factors section** unless a deal asks for it.
...
### 11.4 Cover treatment — DECIDED: clean titled document (golden), deck cover opt-in
The golden HARVEST summary is **plain**: `build_summary.py` emits page-header logo → **title** in `TitleSummary` (big bold) → first H1 (`EXECUTIVE SUMMARY`). **No hero banner, no "INVESTMENT SUMMARY" label, no facts table.** This is the default.

The old **Addepar / Form / Radiant** deck cover is preserved and **opt-in** via `"cover": true`, which restores:
- **Hero banner** — `banner_image` (a white/knockout company logo or a product/facility photo) full content-width; if omitted, a **teal `#1C4849` band** with the company name in white.
...
Either way, section headings render in the template's `SubheaderSummary` style — **small-caps navy with a solid rule** (the Radiant look, which the golden uses). If the Addepar/Form **Title-Case** heading look is preferred instead, turn off `smallCaps` on the `. Subheader (Summary)` style in the template (one-line change) — everything else stays identical.
